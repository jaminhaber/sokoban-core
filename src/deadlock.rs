...
use crate::{direction::Direction, map::Map, math::Vec2, tiles::Tiles};
...
pub fn calculate_static_deadlocks(map: &Map) -> HashSet<Vec2> {
...
pub fn calculate_useless_floors(mut map: Map) -> HashSet<Vec2> {
...
pub fn calculate_useless_boxes(map: &Map) -> HashSet<Vec2> {
    map.box_positions()
        .iter()
        .cloned()
        .filter(|&position| {

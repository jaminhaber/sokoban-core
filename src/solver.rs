...
pub struct Solver {
    map: Map,
    strategy: Strategy,
    lower_bounds: OnceCell<HashMap<Vec2, i32>>,
    tunnels: OnceCell<HashSet<(Vec2, Direction)>>,
}
...
impl Solver {
    ...
    pub fn lower_bounds(&self) -> &HashMap<Vec2, i32> {
    ...
    pub fn tunnels(&self) -> &HashSet<(Vec2, Direction)> {
    ...
    /// the nearest goal.
    fn calculate_minimum_push(&self) -> HashMap<Vec2, i32> {
    ...
    fn calculate_minimum_push_to(
        ...
        lower_bounds: &mut HashMap<Vec2, i32>,
        visited: &mut HashSet<(Vec2, Direction)>,
    ) {
    ...
    fn construct_actions(&self, mut state: State, came_from: &HashMap<State, State>) -> Actions {
        ...
        while let Some(previous_state) = came_from.get(&state) {
            ...
            let push_direction =
                Direction::try_from(Vec2::new(diff.x.signum(), diff.y.signum())).unwrap();

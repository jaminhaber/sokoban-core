...
impl Map {
    ...
    pub fn with_dimensions(dimensions: IVector2) -> Self {
        Self {
    ...
    pub fn player_position(&self) -> IVector2 {
        self.player_position
    }

    /// Sets the position of the player.
    pub fn set_player_position(&mut self, position: IVector2) {
    ...
    pub fn goal_positions(&self) -> &HashSet<IVector2> {
        &self.goal_positions
    }

    /// Sets a box position from one to another.
    pub fn set_box_position(&mut self, from: IVector2, to: IVector2) {
    ...
    pub fn get(&self, position: IVector2) -> Option<&Tiles> {
        self.data
    ...
    /// if out of bounds.
    pub fn get_mut(&mut self, position: IVector2) -> Option<&mut Tiles> {
        self.data
    ...
    pub unsafe fn get_unchecked(&self, position: IVector2) -> &Tiles {
        self.data
    ...
    pub fn can_move(&self, position: IVector2) -> bool {
    ...
    fn remove_box_position(&mut self, position: IVector2) {
        debug_assert!(
            self.box_positions.contains(&position),
    ...
    fn remove_goal_position(&mut self, position: IVector2) {
        debug_assert!(
    ...
    /// the provided border.
    fn flood_fill(&mut self, position: IVector2, value: Tiles, border: Tiles) {
...
impl FromStr for Map {
    ...
    fn from_str(xsb: &str) -> Result<Self, Self::Err> {
        ...
        let mut dimensions = IVector2::zeros();
        ...
        for (y, line) in buf.lines().rev().enumerate() {
            ...
            for (x, char) in line.chars().enumerate() {
                let position = IVector2::new(x as i32, y as i32);
...
impl Index<IVector2> for Map {
    type Output = Tiles;

    fn index(&self, position: IVector2) -> &Tiles {
...
impl IndexMut<IVector2> for Map {
    fn index_mut(&mut self, position: IVector2) -> &mut Tiles {
        assert!(0 <= position.x && position.x < self.dimensions.x);
        assert!(0 <= position.y && position.y < self.dimensions.y);
...
fn calculate_dimensions_and_player_position(actions: &Actions) -> (IVector2, IVector2) {
    ...
    let mut max_position = IVector2::zeros();

    // Calculate the dimensions of the player's and pushed box's movement range
    let mut player_position = IVector2::zeros();
    for action in &**actions {
    ...
    min_position -= IVector2::new(1, 1);
    max_position += IVector2::new(1, 1);
    ...
    let dimensions = min_position.abs() + max_position.abs() + IVector2::new(1, 1);

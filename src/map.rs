...
use crate::{
    ...
    math::Vec2, path_finding::*, run_length::rle_decode, state::State, tiles::Tiles,
};
...
pub struct Map {
    ...
    box_positions: HashSet<Vec2>,
    goal_positions: HashSet<Vec2>,
}
...
impl Map {
    ...
    pub fn with_dimensions(dimensions: Vec2) -> Self {
        Self {
            data: vec![Tiles::empty(); (dimensions.x * dimensions.y) as usize],
            dimensions,
            player_position: Vec2::zeros(),
    ...
    pub fn box_positions(&self) -> &HashSet<Vec2> {
        &self.box_positions
    }

    /// Returns a reference to the positions of the goals.
    pub fn goal_positions(&self) -> &HashSet<Vec2> {
        &self.goal_positions
    }

    /// Sets a box position from one to another.
    pub fn set_box_position(&mut self, from: Vec2, to: Vec2) {
    ...
    pub fn shrink_to_fit(&mut self) {
        ...
        let is_row_empty = |y| {
            let mut row = (0..self.dimensions.x).map(|x| self[Vec2::new(x, y)]);
        ...
        let is_column_empty = |x| {
            let mut column = (0..self.dimensions.y).map(|y| self[Vec2::new(x, y)]);
    ...
    pub fn truncate(&mut self, new_dimensions: Vec2, offset: Vec2) {
    ...
    pub fn get(&self, position: Vec2) -> Option<&Tiles> {
        self.data
    ...
    /// if out of bounds.
    pub fn get_mut(&mut self, position: Vec2) -> Option<&mut Tiles> {
        self.data
    ...
    pub unsafe fn get_unchecked(&self, position: Vec2) -> &Tiles {
        self.data
    ...
    /// [`get_mut`]: Map::get_mut
    /// [undefined behavior]: https://doc.rust-lang.org/reference/behavior-considered-undefined.html
    pub unsafe fn get_unchecked_mut(&mut self, position: Vec2) -> &mut Tiles {
        self.data
    ...
    pub fn can_move(&self, position: Vec2) -> bool {
    ...
    pub fn rotate(&mut self) {
        ...
        let rotate_position = |position: Vec2| Vec2::new(position.y, dimensions.x - 1 - position.x);
    ...
    pub fn flip(&mut self) {
        let dimensions = self.dimensions;
        let flip_position = |position: Vec2| Vec2::new(dimensions.x - 1 - position.x, position.y);
        self.transform(flip_position, self.dimensions);
    }

    /// Adds a box at the given position.
    fn add_box_position(&mut self, position: Vec2) {
        debug_assert!(
    ...
    fn remove_box_position(&mut self, position: Vec2) {
        debug_assert!(
            self.box_positions.contains(&position),
    ...
    fn remove_goal_position(&mut self, position: Vec2) {
        debug_assert!(
    ...
    fn transform(&mut self, operation: impl Fn(Vec2) -> Vec2 + Copy, new_dimensions: Vec2) {
    ...
    /// the provided border.
    fn flood_fill(&mut self, position: Vec2, value: Tiles, border: Tiles) {
...
impl Index<Vec2> for Map {
    ...
    fn index(&self, position: Vec2) -> &Tiles {
...
impl IndexMut<Vec2> for Map {
    fn index_mut(&mut self, position: Vec2) -> &mut Tiles {
        assert!(0 <= position.x && position.x < self.dimensions.x);
        assert!(0 <= position.y && position.y < self.dimensions.y);
...
fn calculate_dimensions_and_player_position(actions: &Actions) -> (Vec2, Vec2) {
    let mut min_position = Vec2::zeros();
    let mut max_position = Vec2::zeros();
    ...
    min_position -= Vec2::new(1, 1);
    max_position += Vec2::new(1, 1);
    ...
    let dimensions = min_position.abs() + max_position.abs() + Vec2::new(1, 1);

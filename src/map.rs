...
use crate::{
    ...
    math::Vector2, path_finding::*, run_length::rle_decode, state::State, tiles::Tiles,
};
...
pub struct Map {
    ...
    goal_positions: HashSet<Vector2>,
}
...
impl Map {
    ...
    pub fn with_dimensions(dimensions: Vector2) -> Self {
        Self {
    ...
    pub fn player_position(&self) -> Vector2 {
    ...
    pub fn box_positions(&self) -> &HashSet<Vector2> {
        &self.box_positions
    }

    /// Returns a reference to the positions of the goals.
    pub fn goal_positions(&self) -> &HashSet<Vector2> {
        &self.goal_positions
    }

    /// Sets a box position from one to another.
    pub fn set_box_position(&mut self, from: Vector2, to: Vector2) {
    ...
    pub fn truncate(&mut self, new_dimensions: Vector2, offset: Vector2) {
    ...
    pub fn get(&self, position: Vector2) -> Option<&Tiles> {
        self.data
    ...
    /// if out of bounds.
    pub fn get_mut(&mut self, position: Vector2) -> Option<&mut Tiles> {
        self.data
    ...
    pub unsafe fn get_unchecked(&self, position: Vector2) -> &Tiles {
        self.data
    ...
    /// [`get_mut`]: Map::get_mut
    /// [undefined behavior]: https://doc.rust-lang.org/reference/behavior-considered-undefined.html
    pub unsafe fn get_unchecked_mut(&mut self, position: Vector2) -> &mut Tiles {
        self.data
    ...
    pub fn can_move(&self, position: Vector2) -> bool {
    ...
    pub fn rotate(&mut self) {
        ...
        let rotate_position = |position: Vector2| Vector2::new(position.y, dimensions.x - 1 - position.x);
    ...
    pub fn flip(&mut self) {
        let dimensions = self.dimensions;
        let flip_position = |position: Vector2| Vector2::new(dimensions.x - 1 - position.x, position.y);
        self.transform(flip_position, self.dimensions);
    }

    /// Adds a box at the given position.
    fn add_box_position(&mut self, position: Vector2) {
        debug_assert!(
    ...
    fn remove_box_position(&mut self, position: Vector2) {
        debug_assert!(
            self.box_positions.contains(&position),
    ...
    fn remove_goal_position(&mut self, position: Vector2) {
        debug_assert!(
    ...
    fn transform(&mut self, operation: impl Fn(Vector2) -> Vector2 + Copy, new_dimensions: Vector2) {
    ...
    /// the provided border.
    fn flood_fill(&mut self, position: Vector2, value: Tiles, border: Tiles) {
...
impl FromStr for Map {
    ...
    fn from_str(xsb: &str) -> Result<Self, Self::Err> {
        ...
        let mut dimensions = Vector2::zeros();
...
impl Index<Vector2> for Map {
    type Output = Tiles;

    fn index(&self, position: Vector2) -> &Tiles {
...
impl IndexMut<Vector2> for Map {
    fn index_mut(&mut self, position: Vector2) -> &mut Tiles {
        assert!(0 <= position.x && position.x < self.dimensions.x);
        assert!(0 <= position.y && position.y < self.dimensions.y);
...
fn calculate_dimensions_and_player_position(actions: &Actions) -> (Vector2, Vector2) {
    let mut min_position = Vector2::zeros();
    let mut max_position = Vector2::zeros();

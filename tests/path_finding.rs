...
fn find_path() {
    ...
    let path = player_move_path(&map, IVector2::new(25, 21)).unwrap();
...
fn test_box_move_waypoints() {
    ...
    assert_eq!(box_move_waypoints(&map, IVector2::new(6, 3)).len(), 0);
    let waypoints = box_move_waypoints(&map, IVector2::new(6, 2));
    let positions: HashSet<_> = waypoints.iter().map(|((pos, _), _)| pos).collect();
    assert_eq!(positions.len(), 15);

    let map = load_level_from_file("assets/Microban II_135.xsb", 132).into();
    let waypoints = box_move_waypoints(&map, IVector2::new(8, 7));
    let positions: HashSet<_> = waypoints.iter().map(|((pos, _), _)| pos).collect();
    let box_path = construct_box_path(IVector2::new(8, 7), IVector2::new(9, 8), &waypoints);
    let player_path = construct_player_path(&map, IVector2::new(7, 6), &box_path);
    ...
    let waypoints = box_move_waypoints(&map, IVector2::new(18, 18));
    let positions: HashSet<_> = waypoints.iter().map(|((pos, _), _)| pos).collect();
    let box_path = construct_box_path(IVector2::new(18, 18), IVector2::new(17, 18), &waypoints);
    let player_path = construct_player_path(&map, IVector2::new(16, 18), &box_path);
    ...
    let waypoints = box_move_waypoints(&map, IVector2::new(16, 34));
    let box_path = construct_box_path(IVector2::new(16, 34), IVector2::new(20, 34), &waypoints);
...
fn test_pushable_boxes() {
    let map = load_level_from_file("assets/Microban_155.xsb", 3).into();
    assert_eq!(pushable_boxes(&map), HashSet::from([IVector2::new(6, 2)]));
}
